...
def filter_segmentation(pcd, lidar_seg, lidar_pose, in_lane_mask=None, point_height=None, max_range=50):
    ...
    for info in lidar_seg["info"]:
        ...
        # Optimization: Fast check using bounding box
        min_xy = np.min(object_points[:, :2], axis=0)
        max_xy = np.max(object_points[:, :2], axis=0)
        if np.linalg.norm(max_xy - min_xy) > 8:
            # If large bbox and too many points, skip to avoid O(N^2) cdist
