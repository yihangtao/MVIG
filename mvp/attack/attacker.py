...
class Attacker():
    ...
    def run(self, case, attack_opts):
        """Base run method that should be called by child classes"""
        try:
            # 验证输入参数
            if not isinstance(case, (list, dict)):
                raise ValueError(f"Invalid case type: {type(case)}")
            if not isinstance(attack_opts, dict):
                raise ValueError(f"Invalid attack_opts type: {type(attack_opts)}")
                
            # 检查必要的字段
            required_fields = ["victim_vehicle_id", "frame_ids"]
            missing_fields = [field for field in required_fields if field not in attack_opts]
            if missing_fields:
                raise ValueError(f"Missing required fields in attack_opts: {missing_fields}")
                
            return self._run_attack(case, attack_opts)
            
        except Exception as e:
            logging.error(f"Error in Attacker.run: {str(e)}")
            logging.error(f"Traceback: {traceback.format_exc()}")
            raise

    def _run_attack(self, case, attack_opts):
